...
<Aside type="tip" title="Setup priority">
Getting storefront setup right is important for both Adobe Commerce as a Cloud Service (ACCS) and Adobe Commerce Optimizer (ACO) customers. Follow each step carefully, especially when linking your repo to commerce data and configuring headers for your backend type.
</Aside>

## Before you start

Identify your Commerce backend type. Configuration differs by backend:

- **Adobe Commerce as a Cloud Service (ACCS)**: Uses standard Commerce headers. See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference (Adobe Commerce as a Cloud Service tab).
- **Adobe Commerce Optimizer (ACO)**: Uses Optimizer-specific headers (`AC-View-ID`, `AC-Source-Locale`, `AC-Price-Book-ID`). See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference (Adobe Commerce Optimizer tab).
...
The Site Creator tool is used to create and initialize the content side of your storefront in the Document Author environment. It creates your content folder and configures the connection between your code repository and content.
...
<Aside type="note" title="Personal vs. Enterprise GitHub accounts">
Setup differs depending on your GitHub account type. Both Adobe Commerce as a Cloud Service (ACCS) and Adobe Commerce Optimizer (ACO) customers use the same storefront creation flow, but GitHub setup varies:
- **Personal account**: Create the repo under your username. The Code Sync app installs to your account. No organization approval needed.
- **Enterprise/Organization**: Create the repo under your organization. You may need admin approval to install the Code Sync app. If your organization uses SAML SSO, you may need to authorize the app for SAML SSO access. Ensure you have permission to create repositories and install GitHub Apps.
</Aside>

<Aside type="note" title="Backend-specific prerequisites">
- **Adobe Commerce as a Cloud Service (ACCS)**: Access to your Commerce Cloud project. GraphQL endpoint from [Catalog Service](https://experienceleague.adobe.com/en/docs/commerce/catalog-service/installation#access-the-service). Store, store view, and website codes from your Commerce Admin.
- **Adobe Commerce Optimizer (ACO)**: Access to Adobe Commerce Optimizer. Catalog view ID, locale, and optionally Price Book ID from the [Adobe Commerce Optimizer UI](https://experienceleague.adobe.com/en/docs/commerce/optimizer/setup/catalog-view). See [Merchandising Services API](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/using-the-api/#headers) for header details.
</Aside>
...
<Aside type="note" title="Personal vs. Enterprise GitHub accounts">
Setup differs slightly depending on your account type:
- **Personal GitHub account**: Create the repo under your personal account. When you select **Owner**, choose your username. The Code Sync app will be installed to your account.
- **GitHub Enterprise (organization)**: Create the repo under your organization. When you select **Owner**, choose the organization. You may need organization admin approval to install the Code Sync app. Ensure you have permission to create repositories and install GitHub Apps in the organization.
</Aside>
...
<Aside type="note" title="Personal vs. Enterprise GitHub accounts">
- **Personal account**: Select your username when choosing where to install. You'll see your personal repositories in the selector.
- **Enterprise/Organization**: Select your organization. You may need admin approval to install the Code Sync app. If your organization uses SAML SSO, you may need to authorize the app for SAML SSO access.
</Aside>
...
To link your storefront repo to your commerce backend, use the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) to generate a `config.json` file. Follow the steps for your backend type:
...
1. Open the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) and enter your Commerce API URL. The tool detects ACCS and generates the appropriate structure.
2. Gather values from your Commerce Admin: store code, store view code, and website code. Get your GraphQL endpoint from the [Catalog Service Guide](https://experienceleague.adobe.com/en/docs/commerce/catalog-service/installation#access-the-service).
3. ACCS uses standard headers (`Magento-Store-Code`, `Magento-Store-View-Code`, `Magento-Website-Code`) and does not require API keys or environment IDs in the default configuration. See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference (Adobe Commerce as a Cloud Service tab) for details.
4. Update the `config.json` file in your repo with the generated or customized values.
5. Commit and push the updated file to the repo.
...
1. Open the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) and enter your Commerce API URL. The tool detects ACO and generates the appropriate structure.
2. Gather values from the [Adobe Commerce Optimizer UI](https://experienceleague.adobe.com/en/docs/commerce/optimizer/setup/catalog-view): `AC-View-ID` (catalog view ID), `AC-Source-Locale`, and optionally `AC-Price-Book-ID`. See the [Merchandising Services API documentation](https://developer.adobe.com/commerce/services/optimizer/merchandising-services/using-the-api/#headers) for header details.
3. See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference (Adobe Commerce Optimizer tab) for the full header structure and any troubleshooting notes.
4. Update the `config.json` file in your repo with the generated or customized values.
5. Commit and push the updated file to the repo.
