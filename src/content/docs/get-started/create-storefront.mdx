...
1. **Add Content** to the Document Author environment hosted at `https://da.live/` using the [Site Creator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator) (also referred to as Storefront Creator in some contexts).
...
The Site Creator tool (sometimes referred to as Storefront Creator) is used to create and initialize the content side of your storefront in the Document Author environment. It creates your content folder and configures the connection between your code repository and content.
...
To link your storefront repo to your commerce backend, you can use the [config generator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/config-generator/config-generator) to automatically generate a `config.json` file for your site based on your Commerce backend.

<Aside type="note" title="Adobe Commerce as a Cloud Service (ACCS) vs. Adobe Commerce Optimizer (ACO)">
Configuration differs by backend type. Use the config generator tool. It detects your backend and generates the appropriate structure. For detailed setup:
- **Adobe Commerce as a Cloud Service**: See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference for Cloud Service endpoints and headers.
- **Adobe Commerce Optimizer**: See the [Storefront configuration](/setup/configuration/commerce-configuration/) reference for Optimizer-specific headers (such as `AC-View-ID`, `AC-Source-Locale`, `AC-Price-Book-ID`).
</Aside>
...
1. Get information about the values required for your project from the [Storefront configuration reference](/setup/configuration/commerce-configuration/) (select the tab for your backend type: Adobe Commerce, Adobe Commerce as a Cloud Service, or Adobe Commerce Optimizer).
...
<Callouts>
1. Open the [Site Creator tool](https://da.live/app/adobe-commerce/storefront-tools/tools/site-creator/site-creator) (also known as Storefront Creator), and select **Use existing repository**.
...
1. Ensure your storefront is configured for [AEM Sites authoring](https://www.aem.live/docs/publishing-from-authoring) if you want to use the full Universal Editor.
2. Open your storefront page in the Universal Editor to edit content in context.
3. For Document Authoring sites, the [Visual Editor](/merchants/quick-start/visual-editor/) provides a subset of Universal Editor features for editing directly on da.live-hosted content.
